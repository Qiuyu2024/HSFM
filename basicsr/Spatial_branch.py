...
class Spatial_branch(torch.nn.Module):
    def __init__(self, dim, d_state,expand,dropout, part1=0.25, part2=0.25,
                 kernels=3,):
        super().__init__()
        self.dim = dim 
        self.global_channels = nearest_multiple_of_16(int(part1 * dim)) 
        if self.global_channels + int(part2 * dim) > dim:
            self.local_channels = dim - self.global_channels
        else:
            self.local_channels = int(part2 * dim)
        ...
        if self.global_channels != 0:
            self.global_op = SS2D(d_model=self.global_channels, d_state=d_state,expand=expand,dropout=dropout, kernels=kernels,) 
        else:
