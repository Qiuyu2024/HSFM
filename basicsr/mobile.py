...
# from basicsr.archs.mambair_arch import SS2D
...
class MBWTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels,d_state,expand,dropout, kernel_size=5, stride=1, wt_levels=1, wt_type='db1'):
        ...
        self.wt_filter, self.iwt_filter = create_wavelet_filter(wt_type, in_channels, in_channels, torch.float) #db1通常称为Haar小波,是最简单的小波
        ...
        self.global_atten = SS2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=dropout, **kwargs)
        #这个看是换成SS2D还是直接删除 先试试换成SS2D，还是可以的
    ...
    def forward(self, x):
        ...
        x = self.base_scale(self.global_atten(x)) #这里调用了SS2D的
        x = x + x_tag
...
class MobileMambaModule(torch.nn.Module):
    def __init__(self, dim, d_state,expand,dropout, global_ratio=0.25, local_ratio=0.25,
                 kernels=3,):
        super().__init__()
        ...
        if self.local_channels != 0:
            self.local_op = DWConv2d_BN_ReLU(self.local_channels, self.local_channels,d_state,expand,dropout,kernels) #使用深度可分离卷积（DWConv2d_BN_ReLU），包含卷积、批归一化和 ReLU 激活。
        else:
            self.local_op = nn.Identity() #如果局部通道数为0，直接进行恒等映射
        if self.global_channels != 0:
            self.global_op = MBWTConv2d(self.global_channels, self.global_channels, kernels, wt_levels=1,) #进行这个操作
        else:
...
class MobileMambaBlock(torch.nn.Module):
    def __init__(self,
                 ed,d_state,expand,dropout, global_ratio=0.25, local_ratio=0.25,
        ...
        self.mixer = Residual(MobileMambaBlockWindow(ed, d_state,expand,dropout,global_ratio=global_ratio, local_ratio=local_ratio,
                                                       kernels=kernels,))
