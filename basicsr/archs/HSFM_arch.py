#Our code is based on MambaIR(A Simple Baseline for Image Restoration with State-Space Model)
import math
import torch
...
from basicsr.Frequency_branch import *
from basicsr.Spatial_branch import *
...
class HFAM(nn.Module): 
    '''
    Hybrid Feature Aggregation Module
    Input: tensor with shape [B, C, H, W]
    Output: tensor with shape [B, C, H, W]
    '''
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.spa_branch = Spatial_branch(dim=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, part1=0.7, part2=0.2)
        self.frequency_branch = frequency_feature(dim=hidden_dim)
        self.drop_path = DropPath(drop_path)
        self.skip_scale= nn.Parameter(torch.ones(hidden_dim))
        self.hfam = HFAM(dim=hidden_dim,hidden_dim=int(2*hidden_dim))
    ...
    def forward(self, input, x_size):
        ...
        spa = self.spa_branch(input.permute(0, 3, 1, 2).contiguous())
        spa = spa.permute(0, 2, 3, 1).contiguous() 
        fre = self.frequency_branch(input.permute(0, 3, 1, 2).contiguous())
        fre = fre.permute(0, 2, 3, 1).contiguous()
        SFMixer = fre + self.drop_path(spa)
        x = self.ln_1(input)
        x = input * self.skip_scale + SFMixer
        x = x * self.skip_scale2 + self.hfam(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3,1).contiguous()  
